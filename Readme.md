...
Production-style backend built as part of the **Chai aur Backend** YouTube series. This project demonstrates how real-world authentication, file uploads, token handling, and security patterns are implemented in a Node.js + Express application.
...
- Cloudinary upload happens next
- Local files are deleted after success or failure
...
- Either set `secure: false`
- Or run backend over HTTPS
...
- **Secure cookies on HTTP**
  - Cookies will not be stored — disable `secure` locally

- **Refresh token typo**
  - Body field is `refreshAcessToken` (matches source code)
...
⭐ If you found this helpful, consider starring the repository and following the series!
